...
function getFacilityZoneOfBlastDoor(blastDoor: mc.Entity): FacilityZone | undefined {
	const belongsToFacilityNetwork =
		blastDoor.getProperty("lc:belongs_to_facility_network",) === true;
...
export function removeBlastDoor(blastDoor: mc.Entity): void {
	...
	try {
		blastDoor.runCommand(
			`fill ^${horSizeDivided} ^${info.size.y} ^0  ^${-horSizeDivided} ^0 ^0 air replace lc:scpdy_blast_door_dummy`,
		);
